"""Persistence ports for atomic Evidence Card reviews."""
...
class ClaimReviewRepository(Protocol):
    def save(self, item: ClaimReviewItem) -> None: ...

    def get(self, item_id: str) -> ClaimReviewItem | None: ...

    def list(self, status: ReviewStatus | None = None) -> list[ClaimReviewItem]: ...


class InMemoryClaimReviewRepository:
    def __init__(self) -> None:
        self._items: dict[str, ClaimReviewItem] = {}
...
class JsonClaimReviewRepository:
    ...
    def _read(self) -> list[ClaimReviewItem]:
        if not self.path.exists():
            return []
        return TypeAdapter(list[ClaimReviewItem]).validate_json(
